...
using namespace osgEarth::Drivers::VTTerrainEngine;
...
namespace
{
    // adapter that lets VTTerrainEngineNode listen to Map events
    struct VTTerrainEngineNodeMapCallbackProxy : public MapCallback
    {
        VTTerrainEngineNodeMapCallbackProxy(VTTerrainEngineNode* node) : _node(node) { }
        osg::observer_ptr<VTTerrainEngineNode> _node;

        void onMapInfoEstablished( const MapInfo& mapInfo ) {
            osg::ref_ptr<VTTerrainEngineNode> node;
            if ( _node.lock(node) )
...
typedef std::map<UID, osg::observer_ptr<VTTerrainEngineNode> > EngineNodeCache;
...
void
VTTerrainEngineNode::registerEngine(VTTerrainEngineNode* engineNode)
{
...
void
VTTerrainEngineNode::unregisterEngine( UID uid )
{
    Threading::ScopedWriteLock exclusiveLock( s_engineNodeCacheMutex );
...
void
VTTerrainEngineNode::getEngineByUID( UID uid, osg::ref_ptr<VTTerrainEngineNode>& output )
{
...
UID
VTTerrainEngineNode::getUID() const
{
    return _uid;
}
...
VTTerrainEngineNode::ElevationChangedCallback::ElevationChangedCallback( VTTerrainEngineNode* terrain ):
_terrain( terrain )
{
    //nop
}
...
void
VTTerrainEngineNode::ElevationChangedCallback::onVisibleChanged( TerrainLayer* layer )
{
...
void
VTTerrainEngineNode::preInitialize( const Map* map, const TerrainOptions& options )
{
...
void
VTTerrainEngineNode::postInitialize( const Map* map, const TerrainOptions& options )
{
    ...
    if ( _primaryUnit < 0 )
    {
        getResources()->reserveTextureImageUnit( _primaryUnit, "VT Engine Primary" );
    }
    ...
    if ( _secondaryUnit < 0 )
    {
        getResources()->reserveTextureImageUnit( _secondaryUnit, "VT Engine Secondary" );
    }
    ...
    map->addMapCallback( new VTTerrainEngineNodeMapCallbackProxy(this) );
...
osg::BoundingSphere
VTTerrainEngineNode::computeBound() const
{
...
void
VTTerrainEngineNode::invalidateRegion(const GeoExtent& extent,
                                      unsigned         minLevel,
                                      unsigned         maxLevel)
{
...
void
VTTerrainEngineNode::refresh(bool forceDirty)
{
...
void
VTTerrainEngineNode::onMapInfoEstablished( const MapInfo& mapInfo )
{
    if ( _update_mapf != 0L )
    {
        dirtyTerrain();
...
osg::StateSet*
VTTerrainEngineNode::getTerrainStateSet()
{
...
void
VTTerrainEngineNode::notifyExistingNodes(TerrainTileNodeCallback* cb)
{
...
osg::StateSet*
VTTerrainEngineNode::getPayloadStateSet()
{
    return _payloadRenderBinPrototype->getStateSet();
}

void
VTTerrainEngineNode::dirtyTerrain()
{
...
void
VTTerrainEngineNode::traverse(osg::NodeVisitor& nv)
{
...
KeyNodeFactory*
VTTerrainEngineNode::getKeyNodeFactory()
{
...
osg::Node*
VTTerrainEngineNode::createNode(const TileKey&    key,
...
osg::Node*
VTTerrainEngineNode::createStandaloneNode(const TileKey&    key,
                                          ProgressCallback* progress)
{
    // if the engine has been disconnected from the scene graph, bail out and don't
    // create any more tiles
...
osg::Node*
VTTerrainEngineNode::createTile( const TileKey& key )
{
...
void
VTTerrainEngineNode::onMapModelChanged( const MapModelChange& change )
{
...
void
VTTerrainEngineNode::addImageLayer( ImageLayer* layerAdded )
{
...
void
VTTerrainEngineNode::removeImageLayer( ImageLayer* layerRemoved )
{
    if ( layerRemoved )
    {
...
void
VTTerrainEngineNode::moveImageLayer( unsigned int oldIndex, unsigned int newIndex )
{
    updateState();
}

void
VTTerrainEngineNode::addElevationLayer( ElevationLayer* layer )
{
...
void
VTTerrainEngineNode::removeElevationLayer( ElevationLayer* layerRemoved )
{
...
void
VTTerrainEngineNode::moveElevationLayer( unsigned int oldIndex, unsigned int newIndex )
{
    refresh();
}

void
VTTerrainEngineNode::toggleElevationLayer( ElevationLayer* layer )
{
    refresh();
}
...
void
VTTerrainEngineNode::updateState()
{
    ...
    else
    {
        ...
        {
            getResources()->reserveTextureImageUnit( _elevationTextureUnit, "VT Engine Elevation" );
        }
        ...
        if ( Registry::capabilities().supportsGLSL() )
        {
            ...
            vp->setName( "osgEarth.engine_vt.TerrainNode" );
            ...
            package.replace( "$VT_PRIMARY_UNIT",   Stringify() << _primaryUnit );
            package.replace( "$VT_SECONDARY_UNIT", Stringify() << (_secondaryUnit>=0?_secondaryUnit:0) );

            package.define( "VT_USE_BLENDING", (_terrainOptions.enableBlending() == true) );
            ...
            bool haveColorFilters = false;
            {
                ...
                std::string fs_colorfilters =
                    ...
                    "$COLOR_FILTER_HEAD"
                    "void oe_vt_apply_filters(inout vec4 color) \n"
                    "{ \n"
                        "$COLOR_FILTER_BODY"
                    "} \n";
