...
#define LC "[VTGeometry] "


VTGeometry::VTGeometry(const TileKey& key, const MapFrame& frame, int imageUnit) :
osg::Geometry    ( ),
_frame           ( frame ),
...
void
VTGeometry::renderPrimitiveSets(osg::State& state,
                                bool        renderColor,
            ...
            for(unsigned i=first; i<_layers.size(); ++i)
            {
                ...
                {       
                    ...
                    for(unsigned int primitiveSetNum=0; primitiveSetNum!=_primitives.size(); ++primitiveSetNum)
                    {
                        ...
                        {
                            OE_WARN << LC << "Strange, VTGeometry had a 0L primset" << std::endl;
                        }
                    }
...
osg::BoundingBox
VTGeometry:: COMPUTE_BOUND() const
{
    osg::BoundingBox bbox = osg::Geometry:: COMPUTE_BOUND ();
    {
        ...
        VTGeometry* ncthis = const_cast<VTGeometry*>(this);
...
void 
VTGeometry::releaseGLObjects(osg::State* state) const
{
...
void
VTGeometry::resizeGLObjectBuffers(unsigned maxSize)
{
...
void 
VTGeometry::compileGLObjects( osg::RenderInfo& renderInfo ) const
{
...
void 
VTGeometry::drawImplementation(osg::RenderInfo& renderInfo) const
{
