...
#include "VTTerrainEngineOptions"
#include "TileNode"
...
#define LC "[engine_vt driver] "

namespace osgEarth { namespace Drivers { namespace VTTerrainEngine
{
    /**
     * osgEarth driver for the VT terrain engine.
     */
    class VTTerrainEngineDriver : public osgDB::ReaderWriter
    {
    public:
        int _profiling;

        VTTerrainEngineDriver()
        {
            _profiling = 0;
            const char* p = ::getenv("OSGEARTH_VT_PROFILE");
            if ( p )
        ...
        {
            return "osgEarth VT Terrain Engine";
        }

        virtual bool acceptsExtension(const std::string& extension) const
        {
            return
                osgDB::equalCaseInsensitive( extension, "osgearth_engine_vt" ) ||
                osgDB::equalCaseInsensitive( extension, "osgearth_engine_vt_tile" ) ||
                osgDB::equalCaseInsensitive( extension, "osgearth_engine_vt_standalone_tile" );
        }

        virtual ReadResult readObject(const std::string& uri, const Options* options) const
        {
            if ( "osgearth_engine_vt" == osgDB::getFileExtension( uri ) )
            {
                ...
                {
                    VTTerrainEngineOptions terrainOpts;
                    OE_INFO << LC << "Activated!" << std::endl;
                    return ReadResult( new VTTerrainEngineNode() );
                }
            }
            else
            {
        ...
        virtual ReadResult readNode(const std::string& uri, const Options* options) const
        {
            ...
            if ( acceptsExtension(ext) )
            {
                ...
                osg::ref_ptr<VTTerrainEngineNode> engineNode;
                VTTerrainEngineNode::getEngineByUID( (UID)engineID, engineNode );
                if ( engineNode.valid() )
                {
                    ...
                    if ( "osgearth_engine_vt_tile" == ext )
                    {
                        node = engineNode->createNode(key, progress);
                    }
                    else if ( "osgearth_engine_vt_standalone_tile" == ext )
                    {
    ...
    REGISTER_OSGPLUGIN(osgearth_engine_vt, VTTerrainEngineDriver);

} } } // namespace osgEarth::Drivers::VTTerrainEngine
